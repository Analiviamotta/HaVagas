...
Os campos que conterão na tela são:
  ...
  Para cada formação os seguintes campos adicionais devem ser
  exibidos:
    o Fundamental e médio: ano de formatura.
    o Graduação e especialização: ano de conclusão e instituição.
    o Mestrado e doutorado: ano de conclusão, instituição, título de monografia e orientador.
    
• Vagas de interesse (campo de texto aberto).

• Botões para salvar e limpar o formulário.

O botão salvar deve mostrar todos os campos que foram preenchidos em uma mensagem pop-up para o usuário. 
Para entradas de texto use as restrições de entrada (número, nome, e-mail, telefone etc.) corretas. 
